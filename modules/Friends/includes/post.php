			...
			switch (Input::get('action')) {
				...
				case 'addFriend':
					if (!isFriends($user->data()->id, $profile[0]->id) && !isRequested($user->data()->id, $profile[0]->id)) {
						...
						$success = str_replace('{x}', $profile[0]->nickname, $language->get('friends', 'friend_request_sent'));
					}
					break;
				...
				case 'removeFriend':
					if (isFriends($user->data()->id, $profile[0]->id)) {
						...
						$success = str_replace('{x}', $profile[0]->nickname, $language->get('friends', 'friend_removed'));
					}
					break;
				...
				case 'acceptRequest':
					if (!isFriends($user->data()->id, $profile[0]->id) && isRequested($profile[0]->id, $user->data()->id)) {
						...
						$success = str_replace('{x}', $profile[0]->nickname, $language->get('friends', 'friend_request_accepted'));
					}
					break;
				...
				case 'cancelRequest':
					if (isRequested($user->data()->id, $profile[0]->id)) {
						...
						$success = str_replace('{x}', $profile[0]->nickname, $language->get('friends', 'friend_request_canceled'));
					}
					break;
			}

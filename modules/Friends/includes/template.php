...
if (isset($user->data()->id) && $user->data()->id !== $profile[0]->id) {
	...
	if (isFriends($user->data()->id, $profile[0]->id)) {
		$friend = array(
			'text' => $language->get('friends', 'remove_friend'),
		...
		$friend = array(
			'text' => $language->get('friends', 'cancel_friend_request'),
		...
		$friend = array(
			'text' => $language->get('friends', 'accept_friend_request'),
		...
		$friend = array(
			'text' => $language->get('friends', 'add_friend'),
			'icon' => '<i class="fas fa-user-plus"></i>',
			'action' => 'addFriend',
		);
	}
...
if (count(listFriends($profile[0]->id))) {
	...
	$smarty->assign('NO_FRIENDS', $language->get('friends', 'no_friends'));